$(document).ready(function() { 
  setTimeout(function(){  
     $("#instafeed .slick-slide").click(function(){
    ...
    jQuery.getJSON('/products/' + product_handle + '.js', function (product) {
      ...
      $(images).each(function (i, image) {
        if (i == imageCount - 1) {
          ...
          image_embed = image_embed.replace('.jpg', '_400x.jpg').replace('.png', '_400x.png');
 ...
 $(window).trigger('resize');
      ...
      $(options).each(function (i, option) {
        ...
        $(option.values).each(function (i, value) {
          ...
          $('.option.' + opt.toLowerCase()).append('<option '+selected+' value="' + value.replace(/['"]/g,'') + '">' + value + '</option>');
          selected = '';
        });
      });
    ...
    $(document).on("change", "#quick-view select", function () {
      ...
      jQuery.getJSON('/products/' + product_handle + '.js', function (product) {
        $(product.variants).each(function (i, v) {
          if (v.title.replace(/['"]/g,'') == selectedOptions) {
    ...
    $.fancybox({
      ...
      'beforeLoad': function () {
        ...
        $(document).on("click", ".qv-add-button", function () {
          ...
          jQuery.getJSON('/products/' + product_handle + '.js', function (product) {
            $(product.variants).each(function (i, v) {
              console.log(v.title.replace(/['"]/g,'') + ' | '+selectedOptions);
              if (v.title.replace(/['"]/g,'') == selectedOptions) {
                var_id = v.id;
                processCart();
              }
          ...
          function processCart() {
            jQuery.post('/cart/add.js', {
              ...
              $('.qv-add-to-cart-response').addClass('success').html('<span>' + $('.qv-product-title').text() + ' נוסף לעגלת הקניות. <a href="/cart">לחץ כאן לצפיה בעגלה.</a>');
            })
            .fail(function ($xhr) {
